...
const UploadFile = ({ projectId, phaseId, workId, moduleId, appId }) => {
    ...
    const onUploadFile = (options) => {
        ...
        const config = {
            onUploadProgress: (progressEvent) =>
                progressNotification(progressEvent, file),
        };

        uploadFile(projectId, phaseId, workId, multipartBody, config).catch(
            (err) => {
    ...
    return (
        <Upload customRequest={onUploadFile} directory={false} fileList={[]}>
