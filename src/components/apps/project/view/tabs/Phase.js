...
function ProjectViewPhase({projectId}) {
    ...
    const onConfirmDelete = (id) => {
        deletePhase(id).then(value => {
            ...
            message.error(err.response?.data?.detail || "Đã có lỗi xảy ra. Vui lòng thử lại sau ít phút!");
        })
    }
    ...
    const mapData = (item) => ({
        ...
        action: <div className={"flex justify-evenly"}>
            ...
            <Popconfirm disabled={item.status !== INACTIVE}
                        title={"Chắc chắn chứ!"}
                        onConfirm={() => onConfirmDelete(item.id)}>
                <Button type={"link"} disabled={item.status !== INACTIVE} icon={<DeleteOutlined/>}/>
            </Popconfirm>
        </div>
    })
